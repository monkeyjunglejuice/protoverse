...
In any case, it makes sense to have straightforward GPU computing available (OpenCL or CUDA) if needed.
** Recursion Support
...
Unforseen complications may likely require to switch certain parts of the implementation during development, e.g. the runtime. It would make sense to write the model in a language that could be ported without much effort. The first what comes to mind are C, C++ and JavaScript. But these are eitber very low-level or ugly, and I'm pretty sure I wouldn't come very far with these as a modeling language. There are two aspects:
1. Use a high-level language that compiles to either a low-level language or can target multiple backends. Examples: [[https://gambitscheme.org/][Gambit Scheme]] -> C, [[https://nim-lang.org/][NIM]] -> C, ... and a growing number of languages target WASM, which could be an interesting runtime, especially with [[https://lunatic.solutions/][projects like this]].
2. Adher to portable code, that means following a standard (e.g. R6RS for the Scheme language).
** Scalability
...
- Hot code reloading: support for upgrading a running application; but it's [[https://learnyousomeerlang.com/relups#the-hiccups-of-appups-and-relups][hellish complicated]]; so it seems that it can't be leveraged for interactive development (preserving state is the goal), and further it does not to compare to Common Lisp's redefining functions and classes within a running system;
...
- Hence the need for massive, high-frequency interactions will still be a challenge (there's no such thing as linar scaling);
- Is said to be slow for CPU-heavy stuff (maybe a matter of concern), however CPU-heavy code could be written as a NIF, e.g. in Rust;
...
- Makes extensive use of metaprogramming (hygienc macros, compile-time only), although it looks quite awkward, implicit and verbose compared to Lisp;
- Lazy streams;
...
- Collab opportunities: Community and ecosystem centered around web backend and boring CRUD business apps; building stuff outside of this domain is slooowly taking off (e.g. machine learning);
...
- Racket lacks in Parallelism: [[https://docs.racket-lang.org/guide/parallelism.html][Places]] is rather hands-on, fully manual;
...
Sentiment: Gerbil Scheme is a promising project but in a very early stage (v18.0), and doesn't support multiple cores. The Emacs integration isn't comparable to Guile's, e.g. documentation or code autocompletion.
** Julia
...
Sentiment: Julia would be an interesting choice and checks many boxes, also because of its library ecosystem centered around scientific computing – if it wasn't totally ignorant about recursion.
** OCaml
...
Sentiment: It's my favorite language actually, but my experience with the messy tooling and the fact there's no real interactivity seems to be the dealbreaker. Static typing is wonderful when you work a lot with 3rd-party libraries, you just look at the tyupes and plug it together. But probably not the right tool for this job.
** Haskell
