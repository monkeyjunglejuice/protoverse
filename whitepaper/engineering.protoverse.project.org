# Created 2025-12-07 Sun 19:34
#+options: 
#+title: Technical Considerations
#+date: <2023-08-22 Tue>
#+author: Dan Dee
#+email: MonkeyJungleJuice@pm.me
...
- No delimited continuations for BEAM languages;
...
- Delimited continuations: somewhat orthogonal to macros -- while metaprogramming (macros) gives you the ability to "step offstage" and change what code exists, delimited continuations let you change how code executes, so that you can build all kinds of weird execution models;
...
- [[https://github.com/opencog/atomspace][OpenCog Atomspace]]: possible backend for the Protoverse with Guile interface,  largely written in Guile and C++; it's a in-RAM generalized hypergraph store and rewriting system;
...
- Has delimited continuations to manipulate control/execution flow;
...
- Has library support for delimited continuations to manipulate control/execution flow;
...
/Sentiment: Julia is an interesting choice and checks many boxes -- the ecosystem fits the domain, and Julia is a good fit for CPU-bound workloads. But I'm not convinced because conceptually, deep recursion will play a crucial role in the Protoverse model, which doesn't translate well to how things are done in Julia./
*** OCaml
...
- Has delimited continuations to manipulate control flow;
...
- Has condition system unwind-protect and whatnot, but no delimited continuations, which are much more general and open the doors to all kinds of interesting things;
- Algebraic data types available as a library, and there is also [[https://github.com/coalton-lang/coalton][Coalton]], an ML-style, internal DSL with type classes);
- Conversational programming at the running system is great for prototyping/exploration;
- Image-based: The running program can be paused, suspended, debugged, modified and continued without losing state -- this is what I really like most about CL;
...
- Concurrency: [[https://github.com/mdbergmann/cl-gserver][Sento]] actor framework is the most promising; and there are [[https://github.com/dbmcclain/Lisp-Actors][Lisp-Actors]] or [[https://github.com/zkat/chanl][ChanL]]. Most concurrency facilities seem to rely directly on OS-threads instead of something "lightweight", though.
...
- SBCL is overall pretty performant (Java ballpark);
...
- High-level: higher-order functions, closures, unrestricted recursion, (delimited?) continuations;
...
After some back- and forth, mainly favoring a BEAM language like Erlang, Elixir or LFE mostly because of BEAM's fine-tuning for concurrency and ad-hoc distribution, I realized that the BEAM has quite a narrow focus and therefore restrictive for conceptual experimentation. I don't want to encounter dead ends during the exploration phase, so I want a language and runtime that's as unlimited as possible and lets me do all kinds of (weird?) stuff, that's why Guile Scheme is my number one candidate.
...
Guile's GC (as of version 3.0.10) seems to need less attention compared to SBCL's current GC (version 2.5.10). When I did some benchmarks with deep non-tail-recursion on binary trees with ~67 million nodes (sequential), Guile turned out 1.5 times faster than SBCL, which spent 3/4 of the time in GC. With annotations and optimization, SBCL tured out a tiny bit faster that Guile.
...
My second candidate should be somewhat orthogonal, and I really like ML family languages, from syntax to type system. A static type system is great for refactoring -- and pattern matching is even more fantastic when it comes packaged with static guarantees, and above all I like how one can model structures concisely with types.

However, common static type systems are only half of the way -- the real thing are dependent types, who let you describe invariants precisely without the clumsy "this list can only have integers" and so on, as that requires you to think around several corners, or multiple turns ahead all the time.

On the other hand, dependent types break down the barrier between interface and implementation. This barrier is a nice thing to have, and dependent types become tricky when it gets more complex. So I'll put OCaml here in the second slot for now, but I'm also going to toy around with dependent types a bit.
