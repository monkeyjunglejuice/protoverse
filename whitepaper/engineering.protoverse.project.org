...
Since I anticipate the Protoverse rather as a long-running application than a batch computation, I lean towards a distributed, horizontal scaling model with redundant nodes that runs in the cloud, and eventually might even run on commodity hardware (e.g. a heterogenous peer-to-peer network), much like Ethereum.
...
- Hot swapping: support for upgrading a running application; but it's said to be complicated (?); can't be leveraged for interactive development afaik.
- Not the same grade of tight-looped interactivity as Common Lisp;
- Function-call level tracing in live applications;
- Recursion is idiomatic: dynamically growing stack depth, no stack overflows (afaik);
...
- Hence the need for massive, high-frequency interactions will still be a challenge;
- Is said to be slow for CPU-heavy stuff (maybe a matter of concern);
- Ad-hoc distributed computing built-in; however distributed Erlang isn't usable to connect nodes over the internet, because there are no security facilities at all. So it's best used only to connect nodes on the same machine or within an isolated cluster.
...
- Livebook for experimentation, visualization, plotting etc.
*** Erlang
...
- No seamless access to libraries from the larger Elixir ecosystem;
- Collab opportunities: Engaged community, focus on infrastructure, industrial-engineering-mindset;

Sentiment: Erlang has no Lisp-style macros, but I think I will need them. And there are some rocks on the way to flowy interactive, explorative programming.
*** Elixir
...
Sentiment: I find Elxir rather inconsistent, verbose and a bit clumsy -- syntax-wise, mostly. There are quite a lot WTFs, but Elixir is more interactive-friendly and adds macros.
...
- Advantages over Elixir/Erlang:
  ...
  - Much closer to Erlang than Elixir, Erlang documentation translates easily;
...
- Documentation is incomplete, fragmented, unpolished and inconsistent;
- Emacs integration is outdated and inferior-lfe needs refactoring (probably I could fix most usability issues in a weekend);
- Eval at runtime is inefficient, because input expressions are evaluated by the LFE interpreter;
- No algebraic data types;
- LFE can't really use Elixir libraries, so there's no real interop with the biggest BEAM ecosystem;
...
Sentiment: Lisp on the BEAM would be a great match if LFE wasn't just a 2-person hobby project.
** Guile Scheme
...
- There's no such thing as a library ecosystem;
...
- Concurrency and multicore [[https://github.com/wingo/fibers/wiki/Manual#13-parallelism][parallelism]] (SMP) via [[https://github.com/wingo/fibers][fibers]] ([[https://github.com/wingo/fibers/wiki/Manual][manual]]), preemptive scheduler;
...
Sentiment: Due to the Fibers library and its scheduler, Guile is quite interesting; but I have no idea how its concurrency capability compares to Erlang, for instance.
** Gerbil Scheme
...
Sentiment: Gerbil Scheme is a promising project but in a very early stage (v18.0), and there's no SMP available. The Emacs integration isn't comparable to Guile's, e.g. documentation or code autocompletion.
** Julia
...
- Recursion is not idiomatic, you'll have to use imperative loops everywhere, Julia's stack depth is capped by the OS;
...
Sentiment: Julia would be an interesting choice because of its library ecosystem centered around scientific computing – if it wasn't totally ignorant about recursion.
** OCaml
...
Sentiment: My experience with the messy tooling and the fact there's no real interactivity seems to be the dealbreaker, even though the language itself is very elegant yet pragmatic.
** Haskell
...
Sentiment: I'm afraid of lazy-per-default, that reasoning about resource usage may become too complicated and a lot time is spent on fighting Haskell's constraints. Seems no great choice for explorative programming.
** JVM: Clojure
...
Sentiment: Dealing with the Java stuff is unpleasant yet unavoidable.
** Common Lisp (SBCL)
...
Both are pretty similar in most regards; the library ecosystem gives Julia an edge, while Common Lisp supports conversational programming at the running application.
