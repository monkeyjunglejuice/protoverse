...
1. Use a high-level language that compiles to either a low-level language or can target multiple backends; for instance, there are many Scheme dialects that target various platforms, and a growing number of languages target WASM, which could be an interesting runtime, especially with [[https://lunatic.solutions/][projects like this]].
